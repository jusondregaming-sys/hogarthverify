...
// ---------------------------------------------------------------------
// Map your Discord role IDs to a rank name. Highest-priority rank first —
// the first match wins, so if someone has multiple roles, put the one
// that should "win" higher in this list.
// ---------------------------------------------------------------------
const RANK_ROLES = [
  { roleId: '1512778519237754970', rank: 'Overseer' },
  { roleId: '1544453130765344828', rank: 'VIP+' },
  { roleId: '1544453072380624977', rank: 'VIP' },
  { roleId: '1544453257496231946', rank: 'Early Supporter' },
];
...
function rankFromRoleIds(roleIds) {
  for (const entry of RANK_ROLES) {
    if (roleIds.includes(entry.roleId)) return entry.rank;
  }
  return DEFAULT_RANK;
}
